...
def send_notification(title: str, message: str) -> None:
    if TOAST_AVAILABLE and _toaster:
        try:
            threading.Thread(
                target=_toaster.show_toast, args=(title, message),
                kwargs={"duration": 5, "threaded": True}, daemon=True,
            ).start()
        except Exception:
            pass
...
DEFAULT_SETTINGS = {
    ...
    "max_tries": 5, "retry_wait": 3, "theme": "dark",
...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        self._s_tries    = ent_row(nt, "Max Retries",                     3, "max_tries")
...
class MainWindow:
    ...
    def _build_list(self):
        ...
        if DND_AVAILABLE:
            try:
                self._scroll.drop_target_register(DND_TEXT, DND_FILES)
                self._scroll.dnd_bind("<<Drop>>", self._on_dnd_drop)
            except Exception: pass
    ...
    def _on_dnd_drop(self, event):
        ...
        if is_valid_url(data): self._start_for_url(data)
        else: self.show_error("Invalid Drop", "Only HTTP/HTTPS URLs are accepted.")
...
class Application:
    def __init__(self):
        ...
        if DND_AVAILABLE:
            try: self.root = TkinterDnD.Tk(); ctk.set_widget_scaling(1.0)
            except Exception: self.root = ctk.CTk()
        else:
            self.root = ctk.CTk()
